...
class HealthReport:
    ...
    def render(self) -> str:
        ordered = (
            "last_poll_attempt", "last_poll_outcome", "last_successful_poll",
            "last_valid_snapshot", "last_source_updated_at", "baseline_initialized",
            "observation_count", "success_count", "fetch_error_count",
            "parse_error_count", "rejected_count", "latest_error_code",
            "quota_state_count", "quota_event_count", "last_successful_email",
        )
        lines = [f"{key}: {self.values[key]}" for key in ordered]
        lines += ["", f"Poller: {self.poller_status}", f"Source: {self.source_status}", f"Email: {self.email_status}"]
        return "\n".join(lines)
...
def build_health_report(
    ...
    *,
    ...
    last_success = _state_datetime(connection, "last_successful_poll")
    last_valid = _state_datetime(connection, "last_valid_snapshot")
    last_email = _state_datetime(connection, "last_successful_email")
    poller_status = "NOT RUN" if last_success is None else ("STALE" if now - last_success > stale_after else "HEALTHY")
    ...
    return HealthReport(values, poller_status, source_status, email_status)
